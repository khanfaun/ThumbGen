exports.handler = async function (event, context) {
  ...
  try {
    ...
    const { url } = body;
    if (!url) {
      return { statusCode: 400, body: JSON.stringify({ error: "Thiếu đường dẫn liên kết Google Drive" }) };
    }
    ...
    if (!/^[a-zA-Z0-9_-]{25,}$/.test(folderId)) {
      return { statusCode: 400, body: JSON.stringify({ error: "Không tìm thấy Folder ID hợp lệ từ đường dẫn được nhập" }) };
    }
    ...
    const unescapeDriveName = (str) => {
      try {
        let result = str.replace(/\\x([0-9a-fA-F]{2})/g, (m, hex) => String.fromCharCode(parseInt(hex, 16)));
    ...
    const extractFolder = async (currentFolderId, currentPath = "") => {
      ...
      if (!response.ok) {
        if (currentFolderId === folderId) throw new Error(`Google Drive API trả về ${response.status}`);
        return;
      }
    ...
    return {
      ...
      body: JSON.stringify({ success: true, folderId, files, count: files.length }),
    };
  } catch (error) {
    return {
      statusCode: 500,
